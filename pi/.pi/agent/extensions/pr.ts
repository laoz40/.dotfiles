...
export default function (pi: ExtensionAPI) {
  function registerPrCommand(name: "pr" | "prdraft", createAsDraft: boolean) {
    pi.registerCommand(name, {
      description: createAsDraft
        ? "Generate a GitHub draft PR body from the project template and insert a gh pr create --draft command. Usage: /prdraft [base-branch] [notes]"
        : "Generate a GitHub PR body from the project template and insert a gh pr create command. Usage: /pr [base-branch] [notes]",
      handler: async (args: string, ctx: ExtensionCommandContext) => {
        ...
        if (filesResult.code !== 0 || commitsResult.code !== 0 || diffResult.code !== 0) {
          ctx.ui.notify(`Could not compare against ${compareRef}. Try /${name} <base-branch>.`, "error");
          return;
        }
        ...
        const command = `!gh pr create${draftFlag} --base ${shellQuote(base)} --head ${shellQuote(branch)} --title ${shellQuote(draft.title)} --body ${shellQuote(draft.body)}`;
        ctx.ui.setEditorText(command);
        notifyDesktop(ctx.cwd, createAsDraft ? "Draft PR message generated." : "PR message generated.");
        ctx.ui.notify(`PR command inserted into Pi chat with the generated description inline. Edit it, then press Enter to create the ${createAsDraft ? "draft PR" : "PR"}.`, "success");
      },
    });
  }

  registerPrCommand("pr", false);
  registerPrCommand("prdraft", true);
}
