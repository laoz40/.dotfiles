...
/**
 * Render percentage plus used/total tokens. If occupancy is unknown, retain
 * the useful capacity as `?% · ?/capacity tokens`; with no valid capacity,
 * omit the statistic entirely.
 */
export function formatContextUtilization(usage: ContextUtilization) {
  ...
  return `${percent === undefined ? "?" : percent}% · ${tokens === undefined ? "?" : formatCompactTokens(tokens)}/${formatCompactTokens(capacity)} tokens`;
}
